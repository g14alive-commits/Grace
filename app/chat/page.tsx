...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(async ({ data }) => {
      ...
      // Check for active session within 3 hours
      ...
      if (activeSession) {
        ...
        // Load messages from localStorage for display
  ...
  // Save messages to localStorage keyed by session
  useEffect(() => {
  ...
  // Check 2 hour active engagement limit
...
const handleSessionClose = async (msgs: string[], uId: string, sId: string) => {
  try {
    ...
    // Add closing message to chat with session-end marker
...
// Detect user ending session
const closingPhrases = ["bye", "goodbye", "ok bye", "thanks bye", "that's all", "thats all", "done for today", "talk later", "see you", "see ya"];
const isClosing = closingPhrases.some(p => userMessage.toLowerCase().includes(p));
if (isClosing && sessionId && userId) {
  setTimeout(() => handleSessionClose(messages, userId, sessionId), 3000);
}
    ...
    try {
      ...
      if (data.result) {
        ...
        // If session closing — generate summary
...
const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
  // On mobile, never intercept Enter — let keyboard handle line breaks naturally
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after {
        ...
        .orb2 {
          width: 280px; height: 280px; bottom: 20%; left: -80px;
        ...
        .header-sub {
          ...
          font-weight: 300; margin-top: 2px; letter-spacing: 0.03em;
        }
        ...
        .download-btn {
          ...
          color: rgba(200,180,255,0.70); transition: all 0.2s; margin-right: 4px;
        }
        ...
        .online-dot {
          ...
          box-shadow: 0 0 8px rgba(100,220,150,0.50); flex-shrink: 0;
        }
        ...
        .msg-label {
          ...
          letter-spacing: 0.10em; text-transform: uppercase; padding: 0 6px;
        }
        ...
        .bubble {
          font-size: 16px; line-height: 1.70; font-weight: 300;
          padding: 14px 18px; border-radius: 20px;
          word-break: break-word; max-width: 90%;
        }

        .bubble.grace {
          background: var(--grace-bubble); border: 1px solid var(--grace-border);
        ...
        .dot {
          ...
          background: var(--dot); opacity: 0.4;
        ...
        textarea {
          ...
          resize: none; height: 24px; max-height: 100px;
  ...
  if (msg === "__SESSION_END__") {
    return (
      <div key={i} style={{
        ...
        <div style={{
          fontSize: "10px", fontWeight: 400, letterSpacing: "0.10em",
          textTransform: "uppercase", color: "rgba(160,140,220,0.40)",
...
{messages.length >= 5 && !showEndSession && (
  <div style={{
    ...
    <button
      ...
      style={{
        ...
        fontSize: "11px",
        color: showEndSession ? "rgba(160,140,220,0.60)" : "rgba(140,130,180,0.40)",
